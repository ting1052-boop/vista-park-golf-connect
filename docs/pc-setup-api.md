...
2. 다음 실행 때 대기 파일이 있으면 운영자에게 전역 토큰을 다시 입력받아 전송한다.
...
1. 마이그레이션 적용 (`202609190001_bay_pc_registry.sql`)
2. Vercel 환경변수 `PC_SETUP_TOKEN` 설정 (32바이트 이상 랜덤)
