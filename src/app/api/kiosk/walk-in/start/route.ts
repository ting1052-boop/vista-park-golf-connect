...
export async function POST(request: NextRequest) {
  ...
  // 결제 연동 전 단계: 결제 완료 처리 버튼(mock)만 인정한다.
  if (body.paymentStatus !== "mock_paid") {
    return NextResponse.json({ ok: false, message: "결제 상태가 올바르지 않습니다." }, { status: 400 });
  }
  ...
  try {
    ...
    for (const bay of freeBays.slice(0, 3)) {
      const { data: inserted, error: insertError } = await supabase
        ...
        .insert({
          ...
          memo: `키오스크 현장 이용, mock payment ${priceByDuration[durationMinutes]}원`
        })
        .select("id")
        .single();
    ...
    if (!reservationId || !assignedBay) {
      return NextResponse.json(
        { ok: false, message: "방금 다른 이용이 시작되어 타석이 마감되었습니다. 다시 시도해주세요." },
        { status: 409 }
      );
    }
